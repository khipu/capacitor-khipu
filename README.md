...
## Behaviour changes in 7.x

Coming from `3.x`. If you are already on `7.0.0`, read the last bullet — that one
release behaved differently and `7.1.0` corrects it.

- **`exitUrl`, `continueUrl` and `failureReason` are now typed `string | null`, and the
  key is always present.** On `3.x` they arrived as `null` on iOS and were *absent* on
